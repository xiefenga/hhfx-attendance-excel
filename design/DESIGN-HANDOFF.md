# d398b733-b2a1-4282-9cfc-0fb6d9cb9340 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `artifact.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
- A single self-contained `artifact.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `artifact.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
## Entry points
- `artifact.html`
- `attendance-summary-prototype.html`
...
1. Inspect `artifact.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
